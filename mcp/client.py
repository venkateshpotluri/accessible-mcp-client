...
from .protocol import MCPCapabilities, MCPError, MCPNotification, MCPRequest, MCPResponse, validate_message, validate_tool_call
...
class MCPClient:
    ...
    def initialize(self, client_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in response:
                raise MCPError(response["error"]["code"], response["error"]["message"], response["error"].get("data"))
    ...
    def _handle_request(self, message: Dict[str, Any]) -> None:
        ...
        error_response = MCPResponse(request_id, error=MCPError(MCPError.METHOD_NOT_FOUND, f"Method {method} not supported"))
